...
main() {
    while true; do
        ...
        [ -z "$selected" ] && {
            log "info" "No resume selected. Exiting."
            exit 0
        }
        ...
        # Show file status
        log "info" "Selected resume: $selected"
        log "info" "Directory: $dir"
        log "info" "TEX file: $([ -f "$tex_file" ] && echo "✓" || echo "✗") $tex_file"
        log "info" "PDF file: $([ -f "$pdf_file" ] && echo "✓" || echo "✗") $pdf_file"
